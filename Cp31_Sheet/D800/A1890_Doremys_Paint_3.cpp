...
public:
     ...
     */
    bool process(int n, int (&arr)[101]) {
        if (n <= 2) return true;
        ...
        // not possible to find k if unique values are more than 2
        if (uset.size() > 2) result = false;
        else if (uset.size() == 2) {
            int freqDiff = 0;
            for(int x: uset)
                freqDiff = abs(freqDiff-freq[x]);
            
            // for even length array, frequency of both the
            // unique elements should be equal
            if (n%2==0 && freqDiff != 0) result = false;
            
            // for odd length array, frequency of both the
            // unique elements should differ by 1
            else if (n%2!=0 && freqDiff != 1) result = false;
        }
