...
public:
    ...
    void print(int n, const T (&arr)[N], const char* end = "\n") {
    ...
    template <size_t N>
    void print(int n, const char (&arr)[N], const char* end = "\n") {
        for (size_t i = 0; i < n; ++i)
            print(arr[i], i + 1 < n ? " " : "");
        cout << end;
    }
...
public:
    ...
    void solve() {
        ...
        vector<int> vec(n);
        for(int& item: vec) cin >> item;

        auto result = process(vec);
        printer.print(result);
        if (result) printer.print(vec);
    }

    bool process(vector<int>& vec) {
        bool result = true;
        ...
        if (min == max) result = false;
        else {
            vec[0] = max;
            vec[vec.size()-1] = vec[1];
            vec[1] = min;
        }
