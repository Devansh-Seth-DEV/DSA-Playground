...
public:
    ...
    void solve() {
        ...
        int binaryArr[101];
        for(int i=0; i<n; i++) cin >> binaryArr[i];

        auto result = process(n, binaryArr);
        printer.print(result);
    }

    int process(int n, int (&binaryArr)[101]) {
        int result = 0;

        int zerosCount = 0;
        for(int i=0; i<n; i++) {
            if (binaryArr[i] == 0) zerosCount++;
            else zerosCount = 0;
            result = max(zerosCount, result);
        }
